...
class TransformerTrainer:
    def __init__(self, model, train_loader, val_loader, dataset, config, device):
        ...
        self.dataset: ZaratustraDataset = dataset
        self.config = config
        self.device = device
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(progress_bar):
            ...
            trgt_ids = batch['trgt_ids'].to(self.device)

            self.optimizer.zero_grad()
            with autocast(device_type='cuda', dtype=torch.float16):
                logits = self.model.forward(src_ids)

                loss = self.criterion(
                    logits[:, :-1].reshape(-1, logits.size(-1)),
                    trgt_ids[:, 1:].reshape(-1)
                )
                ...
                train_bleu += self.compute_bleu(logits.argmax(dim=-1), trgt_ids[:, 1:])
    ...
    def validate(self, epoch):
        ...
        for i, batch in enumerate(tqdm(self.val_loader, desc='Validation')):
            ...
            with torch.no_grad(), autocast(device_type='cuda', dtype=torch.float16):
                generated_ids = self.model.generate_from_ids(src_ids)
